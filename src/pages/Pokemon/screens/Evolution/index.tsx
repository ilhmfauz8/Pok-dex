...
const Evolution: React.FC<{ name: string; color: string }> = ({
  ...
  // Membuat fungsi rekursif untuk menavigasi pohon evolusi pokemon
  // Pada setiap panggilan, nama spesies diekstraksi dan digabungkan dengan daftar nama.
  const handleNameSpecies = useCallback(
    ({
      ...
      // Properti berevolusi_to yang dikembalikan oleh api adalah vektor, dengan cara ini, seseorang dapat
      // pokemon memiliki dua jalur/daun dalam evolusi.
  ...
  useEffect(() => {
    // Di rute /pokemon-species, atribut dari spesies Pokemon dikembalikan
    api.get(`/pokemon-species/${name}`).then(responseSpecies => {
      // Dan di properti evolution_chain diberikan url ke rantai evolusi Pokémon
      // Sebagian url diekstraksi karena aksio sudah memiliki jalur root dari api
  ...
  return (
    <SectionEvolution>
      {evolvesPokemon.length ? (
        ...
        <h1 style={{ textAlign: 'center' }}>Menarik...</h1>
      )}
    </SectionEvolution>
  );
};
